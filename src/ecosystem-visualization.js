...
import { prepareGraphDBWorker, doQuery, prepareVisualizerData/*, queryBindings */ } from "./data"
...
export default async function ecosystem_visualization (base_url) {
  ...
  const graph = ForceGraph3D()
    ...
    .nodeId("project")
    .nodeVal((node) => highlightNodes.has(node.project) || node === selectedNode
      ? node.size * highlightedSizeFactor : node.size
    )
    .nodeColor((node) => highlightNodes.has(node.project)
      ? node === selectedNode ? nodeSelectedColor : nodeHighlightColor
  ...
  prepareGraphDBWorker(base_url);
  const vis = document.createElement('div')
  vis.setAttribute('id', 'graph-viz')
  shadow.append(vis)
  prepareVisualizerData(base_url).then(data => graph(vis).graphData(data))
  // -----------------------------------------------------------------------------------------------------
  return div
}
