...
import { ForceGraphNode } from '../lib/fdg-wasm/fdg-wasm';

export default function NodeVisualizer(nodes: ForceGraphNode[]) {
  // node locations
    ...
    var prefabGeometry = new THREE.IcosahedronGeometry(10.0);
  
    // the number of times the prefabGeometry will be repeated
    var prefabCount = nodes.length;
    ...
    var geometry = new BAS.InstancedPrefabBufferGeometry(prefabGeometry, prefabCount);
    ...
    var aStartPosition: InstancedBufferAttribute = geometry.createAttribute('aStartPosition', 3);
    aStartPosition
    ...
    this.updatePositions = (locations) => {
      // console.log('updating positions for ', geometry)
      for (i = 0; i < prefabCount; i++) {
        aStartPosition
        geometry.setPrefabData(aStartPosition, i, locations[i]);
      }
  ...
  NodeVisualizer.prototype = Object.create(THREE.Mesh.prototype);
  NodeVisualizer.prototype.constructor = NodeVisualizer;
  // helper method for changing the uTime uniform
  Object.defineProperty(NodeVisualizer.prototype, 'time', {
    get: function () {
